...
def plot_dataset(
    data, colors, ax=None, eq_cuts=None, cmap=None, add_colorbar=True, pos=None, gt=None
):
    if data.xs is not None:
        ...
        ax, pos = plot_dataset_graph(
            data.G, data.ys, colors, ax, cmap, add_colorbar, pos, markersize=130
        )
...
def add_colorbar_to_ax(ax, cmap):
    ...
    cb.ax.set_title("p", y=-0.07, fontsize=18)
...
def plot_dataset_metric(
    xs, cs, colors, eq_cuts, ax, cmap, add_colorbar, gt, markersize=80
):
    plt.ioff()
    ...
    if gt is not None:
        ...
        sc = ax.scatter(
            xs_embedded[:, 0], xs_embedded[:, 1], color=colors, s=markersize
        )
...
def plot_soft_predictions(
    ...
    with plt.style.context("ggplot"):
        ...
        if data.ys is not None:
            ...
            if path is not None:
                fig.savefig(output_path / "groundtruth.svg", bbox_inches="tight")
                fig.savefig(output_path / "groundtruth.png", bbox_inches="tight")
                contracted_tree.root.image = path / "groundtruth.png".format(id_node)
                plt.close(fig)
            else:
                plt.show()
...
def plot_soft_prediction_node(
    ...
    if path is not None:
        ...
        fig.savefig(path / "node_nb_{}.pgf".format(id_node), bbox_inches="tight")
        fig.savefig(path / "node_nb_{}.png".format(id_node), bbox_inches="tight")
        node.image = path / "node_nb_{}.png".format(id_node)
        plt.close(fig)
    else:
        plt.show()
